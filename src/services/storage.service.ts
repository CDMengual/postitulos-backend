...
const buildStoragePath = (cohorteId: number, dni: string, tipo: PublicDocumentType, extension: string) => {
...
const isSupportedTipo = (value: string): value is PublicDocumentType => value === 'dni' || value === 'titulo'
...
export const storageService = {
  ...
  async createSignedUploadForPublicForm(params: {
    ...
    if (params.fileSize > maxFileSize) {
      return { error: `El archivo excede el maximo permitido (${maxFileSize} bytes)`, code: 400 as const }
    }
